...
const register = async () => {
  return await request(app).post("/auth/register").send(testUser);
};

const login = async () => {
  return await request(app).post("/auth/login").send(testUser);
};

const registerAndLoginWithAgent = async () => {
  const agent = request.agent(app);
  await agent.post("/auth/register").send(testUser);
  const loginRes = await agent.post("/auth/login").send(testUser);

  return { agent, accessToken: loginRes.body.accessToken };
};
...
describe("POST /auth/register", () => {
  ...
  it("should return 400 if email is invalid", async () => {
    const res = await request(app)
      .post("/auth/register")
      .send({ email: "notemail", password: testUser.password });
...
describe("POST /auth/login", () => {
  ...
  it("should return 401 if password is wrong", async () => {
    ...
    const res = await request(app)
      .post("/auth/login")
      .send({ email: testUser.email, password: "password1234" });
